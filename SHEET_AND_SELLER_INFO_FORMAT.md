...
| H   | Payment Source | Source (e.g. realm)             | Yes              | Options from Payment Info sheet |
| I   | Payment Method | Method                          | Yes              | Options from Payment Info sheet |
| J   | Card Number    | Shomare Kart                    | Yes              | From Seller Info when available |
| K   | Iban           | Shomare Sheba                   | Yes              | From Seller Info when available |
| L   | Name           | Seller name                     | Yes              | From Seller Info when available |
| M   | Wallet         | Wallet address                  | Yes              | From Seller Info when available |
| N   | Paypal Address | Paypal wallet                   | Yes              | From Seller Info when available |
| O   | UUID           | Unique payment ID               | Yes              | 64 hex chars (SHA-256 hash) |
| P   | Note           | User note                       | Yes              | Last column the app writes |
| Q   | Status         | Paid / Unpaid / Failed          | **No** (read-only for app) | App only reads; can be edited in sheet or via “Mark Paid” |

### Important rules

- **App only appends up to column P (Note).** It never writes to column Q or beyond on create. Column Q (Status) can be edited manually in the sheet or via the Dashboard “Mark Paid” / status update.
- **Total** is always `Amount × PPU`; the app overwrites any value in column G.
- **Timestamp** and **Payment Time** are set by the app; no need to fill them manually.
- **UUID** is generated by the app (256-bit hash); do not send or paste it when creating rows via the app.
...
|-------------|--------------|------------------------------|----------------------------|
| Payment v2  | Payment v2   | A–P (Timestamp through Note) | Q (Status)                 |
